                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if($item->upload_type == "image")
                                                    <img class="profile_img" src="{{ !empty (asset('upload/images/'. $item->upload_url)) ? asset('upload/images/'. $item->upload_url) :  "N/A"}}" alt="{{ $item->title}}">
                                                @else
                                                    <video width="100" height="100" controls>
                                                        <source src="{{ !empty (asset('upload/images/'. $item->upload_url)) ? asset('upload/images/'. $item->upload_url) :  "N/A"}}">
                                                    </video>
                                                @endif
                                            </td>
                                            <td class="postDesc">
                                           ...
                                           <td>
                                            <button type="button" class="btn btn-danger btn-sm" onclick="deleteExercise({{ $item->id }})">Delete</button>
                                            <a href="{{ url('exercise/edit/'.$item->id) }}" class="btn btn-primary btn-sm">Update</a>
                                           
                                            <button onclick="exerciseIdAppend({{$item->id}})" type="button" class="btn btn-primary btn-sm exSeconds" data-bs-toggle="modal" data-bs-target="#exampleModal">
                                            Add Exercise Seconds
                                          </button>
                                         <a class="btn btn-primary btn-sm" href="{{route('exercise.seconds')}}"> View Exercise Seconds</a>
                                        </td>
                                        </tr>
                                        @endforeach
                                        @endif
